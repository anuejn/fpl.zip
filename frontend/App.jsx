...
export default function App(props) {
  const { loading, error, data = [] } = useFetch(
    "https://files.niemo.de/kondome.mp3.json",
    {},
    []
  );
  ...
  useEffect(() => {
    document.onselectionchange = () => {
      ...
      if (data) {
            ...
            return [
              {
                ...
                endIndex: lastIndex + curr.word.length + 1,
                ...curr,
              },
              ...acc,
            ];
        ...
        const words = tagged_data.filter(
          (word) =>
            word.endIndex - 1 > range.startOffset &&
            word.startIndex < range.endOffset
        );
        console.log(words.map((x) => x.word));
        const [start, end] = [
          Math.min(...words.map((x) => x.start)),
          Math.max(...words.map((x) => x.end)),
        ];
